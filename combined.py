...
def wnd_proc(hwnd, msg, wparam, lparam):
    ...
    if msg == win32con.WM_PAINT:
        ...
        # If overlay_text is empty, default to "Hello World"
        text_to_draw = overlay_text if overlay_text else "Hello World"
        rect = win32gui.GetClientRect(hwnd)
        x, y = rect[0], rect[1]

        for line in text_to_draw.splitlines():
            # Draw each line at the current (x, y)
            win32gui.DrawText(
                hdc,
                line,
                -1,
            ...
            # Measure this line's height so we know where to start the next line
            (line_width, line_height) = win32gui.GetTextExtentPoint32(hdc, line)
            y += line_height
